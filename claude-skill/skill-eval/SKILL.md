...
우선순위: 사용자 지정 > 기존 등록 태스크 > SkillsBench 적합 태스크 > (질문 후) 직접 생성.

1. 사용자가 태스크(디렉토리)를 지정했으면 그것을 사용 (`tasks/` 밖이면 복사해 등록).
2. 아니면 `tasks/`에서 `metadata.yaml`의 `required_skill == <skill-id>`인 기존 태스크 탐색.
3. 없으면 `upstream/skillsbench/tasks/`(87개)에서 **적합한 태스크를 찾아 우선 사용**한다.
   적합 기준: (i) 태스크 주제·요구 역량이 스킬이 다루는 업무와 일치,
   (ii) 로컬 실행 가능(`requires.network == none` 또는 네트워크 없이 채점 가능,
   Docker 전용 아님). 적합 후보가 있으면 질문 없이 변환해 쓰되,
   어떤 태스크를 골랐고 왜 적합한지 보고에 명시한다:
   `python -m skill_eval.cli import-sb --task upstream/skillsbench/tasks/<id> --required-skill <skill-id>`
   (외부 출제라 자작 태스크보다 효과 과장 편향이 없음 — 그래서 우선.)
4. SkillsBench에도 적합한 것이 없는 스킬만 **생성 전 반드시 사용자에게 질문**한다.
   텍스트로 질문하고(이 환경에선 AskUserQuestion 도구가 취소될 수 있음),
   **복수 스킬이어도 질문은 한 번만 묶어서** 한다:

   > 평가용 태스크가 없는 스킬: `A`, `B` (SkillsBench에도 적합 태스크 없음).
   > 스킬별 3개씩(난이도 하/중/상) 태스크를 생성할까요? 개수·난이도 조정도 가능합니다.
