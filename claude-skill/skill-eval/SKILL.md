...
### 2.5 정적 진단 (선택 — 사용자가 "정적으로", "추산만", "lint" 요청 시 또는 실측 전 스크리닝)

```bash
python -m skill_eval.cli lint --skill <스킬 경로> [--skill ...]
```

실행 없이 SKILL.md 구조를 9개 항목으로 채점(0~100): 발동 조건, 단계화, 모호어,
검증 절차, 오류 대응, 산출물 명시, 자원 참조 일치, 분량 오버헤드, 제약 정의.
비용 0. **보고 시 "추정이며 실측 아님"을 반드시 명시.** 점수가 낮은 스킬은
실측 전 개선 포인트를 사용자에게 먼저 보여준다. 사용자가 정적 진단만 원했다면
여기서 종료.
