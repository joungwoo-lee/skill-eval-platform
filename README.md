...
Skill Lift(성공률 %p)·성공당 시간·비용은 구성 지표로 함께 보고된다.
...
- **LLM 판정**: 같은 루브릭 + 패턴이 못 보는 3항목(내용 타당성·내부 일관성·절차 충분성)을
  의미 기반으로 채점. 기본은 **셀프 판정**(`--judge-file`) — 플랫폼을 구동하는 에이전트가
  이미 LLM이므로 본인이 채점한 JSON을 전달, 별도 모델 호출 없음. `--judge`는
  에이전트 밖 환경용 headless claude 폴백.
